...
hiddenimports = [
    "backend.api", "backend.cad", "backend.language_assets", "backend.licensing", "backend.providers.azure", "backend.queue", "backend.storage", "backend.text_cleaning", "backend.translator", "backend.updater", "desktop.launcher", "desktop.native_bridge", "python_multipart",
...
pyz = PYZ(a.pure)
exe = EXE(pyz, a.scripts, a.binaries, a.datas, [], name="Honsen DrawTranslate v1.9.5", console=False, icon=[os.path.join(spec_dir, "ico.ico")])
