# Honsen CAD 中法英互译工具 v1.7.1

面向建筑、结构和机电图纸的 Windows 桌面翻译工具。它读取 CAD 图纸文字，使用 DeepL 与工程术语表生成独立的译文图纸，支持单文件和可恢复的批量翻译队列。
...
- 内置中法、法中、中英、英中 CAD 术语表。完整标签命中术语表时直接采用术语译文；其余文字交由 DeepL 翻译。
...
1. 启动程序，填写 DeepL API Key，选择输出目录、统一翻译方向、输出格式和版本。
2. 点击“添加文件”选择一个或多个 DXF/DWG 文件；添加不会立即调用 ODA 或 DeepL。
3. 点击“开始翻译”。处理中可暂停或停止，失败项可单独重翻。
4. 在输出目录中查看带语言前缀的译文图纸，并在 CAD 软件中复核文字与版式。

DeepL Key 和默认输出目录仅保存在用户目录的 `~/.cad_translator_config.json`；也可通过环境变量 `DEEPL_API_KEY` 提供 Key。
...
然后访问 `http://localhost:5173`。旧版 Tkinter 界面可使用 `python main.py --legacy` 启动。
...
npm run build
cd ..
pyinstaller --clean --noconfirm Honsen_CAD_Translator_v1.7.1.spec
```

生成文件为 `dist/Honsen_CAD_Translator_v1.7.1.exe`。如需开箱支持 DWG，请将完整 ODA 目录放在 `dist/ODAFileConverter/`。
