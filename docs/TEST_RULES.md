...
Packaging status: package with `pyinstaller --clean --noconfirm Honsen_CAD_Translator_v1.7.1.spec` and verify `dist/Honsen_CAD_Translator_v1.7.1.exe`. Before external distribution, perform a desktop launch smoke test with the adjacent local `ODAFileConverter/` directory.
...
Shutdown enforcement: after state persistence and cancellation, the close control must force the application process to exit; verify no `Honsen_CAD_Translator_v1.7.1.exe` process remains after closing the window.
